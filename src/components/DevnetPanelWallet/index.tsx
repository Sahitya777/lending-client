...
const DEFAULT_TOKENS: TokenConfig[] = [
  ...
  { symbol: 'USDC', mint: '4zMMC9srt5Ri5X14GAgXhaHii3GnPAEERYPJgZJDncDU', usdPrice: 1.0, programId: 'token' },
  // Example extra (comment in if you want USDT Devnet)
  { symbol: 'USDT', mint: 'Ejmc1UB4EsES5UfZyaG7kRGtCLGnogT7xWRqvEih5Z7', usdPrice: 1.0, programId: 'token' },
]
...
export default function DevnetWalletPanel({
  ...
  const [rows, setRows] = useState<{ symbol: string; amount: number; usdPrice?: number }[]>([])
    ...
    async function run() {
      ...
      try {
        ...
        if (solCfg) {
          ...
          results.push({ symbol: solCfg.symbol, amount: lamports / LAMPORTS_PER_SOL, usdPrice: solCfg.usdPrice })
        }
        ...
        if (nonNative.length) {
          ...
          const allParsed = await connection.getParsedTokenAccountsByOwner(owner, {
            programId: TOKEN_PROGRAM_ID,
          })
          // Optional: also check Token-2022 program in case some devnet mints are 2022
          const allParsed2022 = await connection.getParsedTokenAccountsByOwner(owner, {
          ...
          for (const cfg of nonNative) {
            ...
            const match = all.find(a => a.account.data?.parsed?.info?.mint === mintStr)
            let amount = 0
            if (match) {
              const ui = match.account.data.parsed.info.tokenAmount.uiAmount
              amount = typeof ui === 'number' ? ui : 0
            }
            results.push({ symbol: cfg.symbol, amount, usdPrice: cfg.usdPrice })
          }
        }
  ...
  const totalUSD = useMemo(() => {
    return rows.reduce((acc, r) => acc + (r.usdPrice ? r.amount * r.usdPrice : 0), 0)
  }, [rows])
    ...
    <div className="space-y-6 w-full">
      ...
      <div className="rounded-2xl border border-gray-200 bg-white shadow-sm">
        ...
        {address && (
          <div className="px-5 pb-5">
            ...
            {loading && (
              <div className="px-1 py-3 text-sm text-gray-500">Loading balances…</div>
            )}
            {error && (
            ...
            <div className="divide-y">
              {rows.map(r => {
                ...
                return (
                  <div key={r.symbol} className="grid grid-cols-4 items-center px-1 py-3 text-sm">
                    ...
                    <div className="text-right">{r.usdPrice != null ? `$${r.usdPrice.toFixed(2)}` : '—'}</div>
                    <div className="text-right">{r.amount.toLocaleString(undefined, { maximumFractionDigits: 6 })}</div>
                    <div className="text-right">{value != null ? `$${value.toFixed(2)}` : '—'}</div>
                  </div>
                )
              })}
            </div>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white shadow-sm">
        <div className="flex items-center justify-between p-5 border-b">
          ...
          <button className="rounded-lg p-1 hover:bg-gray-100 transition" aria-label="collapse">
            <ChevronUp className="h-4 w-4 text-gray-500" />
          </button>
        </div>
        <div className="p-2">
          {[
            { label: 'Lend SOL', apy: 149.28 },
            { label: 'Lend USDT', apy: 128.94 },
            { label: 'Lend USDC', apy: 128.61 },
          ].map(op => (
            <div key={op.label} className="flex items-center justify-between px-3 py-3 text-sm hover:bg-gray-50 rounded-xl">
              <div className="flex items-center gap-3">
              ...
              <div className="text-right font-medium">{op.apy.toFixed(2)}%</div>
            </div>
          ))}
        </div>
      </div>
...
export function useDevnetBalances(args: {
  ...
  const { address, rpcUrl = clusterApiUrl('devnet'), tokenConfigs = DEFAULT_TOKENS } = args
  const [state, setState] = useState<{ loading: boolean; error?: string; rows: { symbol: string; amount: number; usdPrice?: number }[] }>({ loading: false, rows: [] })
  useEffect(() => {
    let cancelled = false
    async function run() {
      if (!address) return
      ...
      try {
        ...
        const allParsed = await connection.getParsedTokenAccountsByOwner(owner, { programId: TOKEN_PROGRAM_ID })
        const allParsed2022 = await connection.getParsedTokenAccountsByOwner(owner, { programId: TOKEN_2022_PROGRAM_ID }).catch(() => ({ value: [] as any[] }))
        const all = [...allParsed.value, ...allParsed2022.value]
        for (const cfg of tokenConfigs.filter(t => !t.native && t.mint)) {
          const match = all.find(a => a.account.data?.parsed?.info?.mint === cfg.mint)
          const ui = match?.account?.data?.parsed?.info?.tokenAmount?.uiAmount
          results.push({ symbol: cfg.symbol, amount: typeof ui === 'number' ? ui : 0, usdPrice: cfg.usdPrice })
        }
        if (!cancelled) setState({ loading: false, rows: results })
      } catch (e: any) {
        if (!cancelled) setState({ loading: false, rows: [], error: e?.message || 'Failed to load balances' })
      }
    }
    run()
